"""CatBoost experiment 3 -- Ratio features.

Adds domain-motivated ratio features (e.g. rain per hectare,
moisture-to-humidity ratio) on top of baseline features.
"""
...
OUTPUT_PATH = f"{SAVE_DIR}/cat_3.csv"
OOF_PATH = f"{SAVE_DIR}/cat_3_oof.csv"
TEST_PROBS_PATH = f"{SAVE_DIR}/cat_3_test.csv"
...
def add_ratios(df):
    ...
    df["prev_irr_over_area"] = (
        df["Previous_Irrigation_mm"] / (df["Field_Area_hectare"] + eps)
    )
    df["carbon_over_pH"] = df["Organic_Carbon"] / (df["Soil_pH"] + eps)
    df["EC_over_moisture"] = (
        df["Electrical_Conductivity"] / (df["Soil_Moisture"] + eps)
    )
    df["moisture_over_temp"] = (
        df["Soil_Moisture"] / (df["Temperature_C"] + eps)
    )
    df["humidity_over_wind"] = (
        df["Humidity"] / (df["Wind_Speed_kmh"] + eps)
    )
    df["rain_over_prev_irr"] = (
        df["Rainfall_mm"] / (df["Previous_Irrigation_mm"] + eps)
    )
    return df
...
for func in [add_decimal_digits, add_binary_flags, add_ratios]:
    X = func(X)
    X_test = func(X_test)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
    ...
    cw = compute_class_weight(
        "balanced", classes=np.sort(y_tr.unique()), y=y_tr
    )
