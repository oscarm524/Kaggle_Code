"""CatBoost experiment 2 -- Interaction features.

Adds pairwise multiplicative interactions among the most important
numeric features on top of the baseline decimal-digit + binary-flag
feature set.
"""
...
OUTPUT_PATH = f"{SAVE_DIR}/cat_2.csv"
OOF_PATH = f"{SAVE_DIR}/cat_2_oof.csv"
TEST_PROBS_PATH = f"{SAVE_DIR}/cat_2_test.csv"
...
def add_decimal_digits(df):
    for col in NUMERIC_COLS:
        ...
        df[f"{col}_dec"] = np.floor((values - np.floor(values)) * 10).astype(int)
    return df
