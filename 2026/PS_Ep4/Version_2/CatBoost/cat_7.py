"""CatBoost experiment 7 -- Second decimal digit + quantile binning.

Extracts the second decimal digit (captures deeper synthetic-data
patterns) and adds quantile-binned versions of key numeric features.
"""
...
OUTPUT_PATH = f"{SAVE_DIR}/cat_7.csv"
OOF_PATH = f"{SAVE_DIR}/cat_7_oof.csv"
TEST_PROBS_PATH = f"{SAVE_DIR}/cat_7_test.csv"
...
def add_quantile_bins(train_df, test_df, cols, n_bins=10):
    """Quantile-bin numeric features."""
    for col in cols:
        train_df[f"{col}_qbin"], bins = pd.qcut(
            train_df[col], q=n_bins, labels=False,
            retbins=True, duplicates="drop"
        )
        test_df[f"{col}_qbin"] = pd.cut(
...
bin_cols = ["Soil_Moisture", "Temperature_C", "Rainfall_mm",
            "Humidity", "Previous_Irrigation_mm"]
X, X_test = add_quantile_bins(X, X_test, bin_cols, n_bins=10)
