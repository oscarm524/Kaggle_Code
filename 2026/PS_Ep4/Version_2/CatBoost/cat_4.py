"""CatBoost experiment 4 -- Log transforms + polynomial features.

Applies log1p transforms to skewed numeric columns and adds
...
OUTPUT_PATH = f"{SAVE_DIR}/cat_4.csv"
OOF_PATH = f"{SAVE_DIR}/cat_4_oof.csv"
TEST_PROBS_PATH = f"{SAVE_DIR}/cat_4_test.csv"
...
def add_log_poly(df):
    """Log transforms for skewed cols + squared features."""
    skewed = ["Rainfall_mm", "Field_Area_hectare",
              "Previous_Irrigation_mm", "Electrical_Conductivity"]
    for col in skewed:
        df[f"{col}_log"] = np.log1p(df[col])

    squared = ["Soil_Moisture", "Temperature_C", "Humidity",
...
for func in [add_decimal_digits, add_binary_flags, add_log_poly]:
    X = func(X)
    X_test = func(X_test)
