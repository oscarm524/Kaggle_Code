...
lgb_params = {
    'learning_rate': 0.1124121813710808,
    'max_depth': 4,
    'reg_alpha': 9.710588297053341,
    'reg_lambda': 0.30919961518106104,
    'num_leaves': 26,
    'colsample_bytree': 0.6009768997648857,
    'subsample': 0.890955039393104,
    'min_child_samples': 31,
    'verbose': -1,
    'n_jobs': -1,
    'device': 'gpu',
    'smoothing': 0.3726080918817735,
    'noise_level': 0.01646934814667607
}
...
for i, (train_index, test_index) in enumerate(skf.split(X, y)):
    ...
    test_preds["lgb_pred_7"] = test_pred
    test_preds["fold"] = i
...
preds_per_fold = [df["lgb_pred_7"].values for df in test_preds_df]
...
submission.to_csv("lgb_sub_7.csv", index=False)

print("Saving OOF predictions...")
oof_preds_df = pd.concat(oof_preds_df)
oof_preds_df.to_csv("oof_lgb_7.csv", index=False)

print("Saving test predictions...")
test_preds_df = pd.concat(test_preds_df)
test_preds_df.to_csv("test_lgb_7.csv", index=False)
print("Done!")
