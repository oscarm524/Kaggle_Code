...
lgb_params = {
    'learning_rate': 0.09765029857418714,
    'max_depth': 6,
    'reg_alpha': 9.543498850760129,
    'reg_lambda': 8.495250045623626,
    'num_leaves': 23,
    'colsample_bytree': 0.603870803458922,
    'verbose': -1,
    'n_jobs': -1,
    'device': 'gpu'
}
...
preds_per_fold = [df["lgb_pred_4"].values for df in test_preds_df]
...
submission.to_csv("lgb_sub_4.csv", index=False)

print("Saving OOF predictions...")
oof_preds_df = pd.concat(oof_preds_df)
oof_preds_df.to_csv("oof_lgb_4.csv", index=False)

print("Saving test predictions...")
test_preds_df = pd.concat(test_preds_df)
test_preds_df.to_csv("test_lgb_4.csv", index=False)
print("Done!")
